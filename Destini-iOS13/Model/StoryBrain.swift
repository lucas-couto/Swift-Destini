...
struct StoryBrain{
  ...
  var storyLocal: Int = 0
  
  func getStory() -> Story{
    return stories[storyLocal]
  }

  
  mutating func nextStory(_ choice: String){
    if(stories[storyLocal].choice1 == choice){
      storyLocal = stories[storyLocal].choice1Destination
    }else{
      storyLocal = stories[storyLocal].choice2Destination
    }
  }
